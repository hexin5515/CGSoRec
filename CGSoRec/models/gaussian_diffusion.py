...
class GaussianDiffusion(nn.Module):
    ...
    def condition_mean(self, cond_fn, cond, p_mean_var, x_t, t):
        """
        ...
        """
        # gradient = cond_fn(x_t, t, cond)
        lamda = 0.1
        new_mean = (
            (1 - lamda) * p_mean_var["mean"].float() + lamda * cond.float()
        )
        return new_mean
    
    def p_sample(self, model, x_start, cond, cond_fn = None, steps = None, sampling_noise=False):
        ...
        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            cond_out = self.p_mean_variance(model, cond, t)
            ...
            if sampling_noise:
                ...
                if cond_fn is not None:
                    out["mean"] = self.condition_mean(cond_fn, cond_out["mean"], out, x_t, t)
                x_t = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
            else:
                if cond_fn is not None:
                    out["mean"] = self.condition_mean(cond_fn, cond_out["mean"], out, x_t, t)
                x_t = out["mean"]
                # cond_t = cond_out["mean"]

            # x_t = x_t * 0.99 + cond_t * 0.01
            
        return x_t
    
    def p_sample_train(self, model, x_start, steps = None, sampling_noise=False):
        assert steps <= self.steps, "Too much steps in inference."
        if steps == 0:
            x_t = x_start
        else:
            t = th.tensor([steps - 1] * x_start.shape[0]).to(x_start.device)
            # noise = th.randn_like(x_start).mul(batch_2)
    ...
    def p_sample_inference(self, model, x_start, cond, steps = None, sampling_noise=False, cond_weigth=0):
        assert steps <= self.steps, "Too much steps in inference."
        if steps == 0:
            x_t = x_start
            x_t_c = cond
        ...
        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            out = self.p_mean_variance(model, x_t, t)
            out_cond = self.p_mean_variance(model, x_t_c, t)
            if sampling_noise:
                noise = th.randn_like(x_t)
                nonzero_mask = (
                    (t != 0).float().view(-1, *([1] * (len(x_t.shape) - 1)))
                ...
                x_t = ((1-cond_weigth) * out["mean"] + cond_weigth * out_cond["mean"])/2.
        return x_t
    ...
    def training_losses(self, model, x_start, reweight=False):
        ...
        # noise = th.randn_like(x_start).mul(th.zeros_like(x_start))
        # print(noise.nonzero())
        if self.noise_scale != 0.:
    ...
    def training_losses_cl(self, model, x_start, cond, reweight=False):
        batch_size, device = x_start.size(0), x_start.device
        ts, pt = self.sample_timesteps(batch_size, device, 'uniform')
        noise = th.randn_like(x_start)
        if self.noise_scale != 0.:
            x_t = self.q_sample(x_start, ts, noise)
            cond_t = self.q_sample(cond, ts, noise)
        else:
            x_t = x_start
            cond_t = cond
        ...
        x_t_emb = model.encoder_list(x_t, ts)
        cond_t_emb = model.encoder_list(cond_t, ts)
        dis_loss = th.exp(th.mm(x_t_emb, cond_t_emb.t())).diag().sum()
        ...
        terms["loss"] += 1e-3 * dis_loss
        return terms
    
    def social_training_losses(self, model, x_start):
        batch_size, device = x_start.size(0), x_start.device
        ts, pt = self.sample_timesteps(batch_size, device, 'uniform')
        noise = th.randn_like(x_start)
        if self.noise_scale != 0.:
    ...
    def encoder_training_losses(self, model, x_start):
        device = x_start.device
        
        t = th.randint(0, 1, (x_start.size(0),), device=device).long()
    ...
    def encoder_training_fun_losses(self, model_1, model_2, x_start):
        batch_size, device = x_start.size(0), x_start.device
        ts, pt = self.sample_timesteps(batch_size, device, 'uniform')
        noise = th.randn_like(x_start)
        if self.noise_scale != 0.:
            ...
            x_t = x_start
        terms = {}
        t = th.randint(0, 1, (batch_size,), device=device).long()
        with th.no_grad():
            emb_start = model_1.encoder_list(x_start, t)
        
        emb_t = model_2.encoder_list(x_t, ts)

        emb_start_norm = emb_start / th.norm(emb_start, dim=1, keepdim=True) #x0 batch表示
        emb_t_norm = emb_t / th.norm(emb_t, dim=1, keepdim=True) #xt batch

        s_value = th.exp(th.mm(emb_t_norm, emb_t_norm.t()) / self.tau)
        # print(s_value)
        b_value = th.exp(th.mm(emb_start_norm, emb_t_norm.t()) / self.tau)
        neg_mask = th.eye(batch_size, dtype=th.float32).to(device)
        ...
        value_neg = value_neg - value_zi * neg_sum
        value_neg = th.max(value_neg, neg_sum * math.exp(-1.0 / self.tau))
        value_mu = value_zi + value_neg
        
        # print(value_mu)
        loss = -th.log(value_zi / value_mu)

        terms['loss'] = loss
        return terms
        # emb_start_norm = emb_start / th.norm(emb_start, dim=1, keepdim=True)
        # emb_t_norm = emb_t / th.norm(emb_t, dim=1, keepdim=True)

        # cos_similarity = th.mm(emb_start_norm, emb_t_norm.t())

        # mask = th.eye(batch_size, dtype=th.float32).to(device)
        # # logits_max, _ = th.max(cos_similarity, dim=1, keepdim=True)
        # logits = cos_similarity  # - logits_max.detach()
        ...
        # # num_positives_per_row  = th.sum(positives_mask, axis=1)  # 除了自己之外，正样本的个数  [2 0 2 2]       
        # denominator = th.sum(exp_logits * negatives_mask, axis=1, keepdims=True) + th.sum(
        #     exp_logits * positives_mask, axis=1, keepdims=True) 
        
        # log_probs = logits - th.log(denominator)
    ...
    def p_mean_variance(self, model, x, t):
        ...
        # model_mean = self._extract_into_tensor(self.sqrt_alphas_cumprod, t,  pred_xstart.shape) * pred_xstart 
